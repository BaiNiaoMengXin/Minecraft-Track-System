...
export namespace LZString {
    ...
    const baseReverseDic: Record<string, any> = {};
    ...
    export const compressToBase64 = (input: string) => {
    ...
    export const decompressFromBase64 = (input: string) => {
        if (input == null) return "";
        if (input == "") return null;
        return _decompress(input.length, 32, function (index) { return getBaseValue(keyStrBase64, input.charAt(index)); });
    }

    const _compress = (uncompressed: string, bitsPerChar: number, getCharFromInt: (num: number) => string) => {
    ...
    const _decompress = (length: number, resetValue: number, getNextValue: (a: number) => number) => {
        var dictionary = [],
            next,
            enlargeIn = 4,
            dictSize = 4,
